// Hosts pdf.js + @napi-rs/canvas in its own OS process, forked by pdfExtractor.js. A wedged
// render or parse here (pdf.js's Node/worker-thread misdetection makes those indistinguishable
// from a slow one) can only ever freeze this process, never the worker's tick loop —
// pdfExtractor.js recovers a hang by SIGKILLing this whole process, which the kernel guarantees
// works even if this process is blocked in a way no in-process Promise.race timeout could
// preempt (the same reasoning ocrEngine.js/ocrChildProcess.js already apply to tesseract).
const fs = require('fs');
...
// forked child process, so pdf.js thinks it's *not* in Node and skips the polyfill entirely.
// Rendering a page that touches image compositing (e.g. `putBinaryImageData`) then references
// the bare `ImageData`/`Path2D`/`DOMMatrix` globals directly and throws "X is not defined".
// Setting them ourselves sidesteps pdf.js's broken environment detection.
...
// when process.versions.electron is set. This forked child reports a different process.type,
// so pdf.js misdetects it as a browser context and reaches for DOM/fetch-based readers that
// don't exist here (surfacing as "document is not defined"). These two small classes replicate
// pdf.js's own NodeCMapReaderFactory/NodeStandardFontDataFactory (plain fs.readFile against a
// local directory) so we can force the Node behavior explicitly instead.
